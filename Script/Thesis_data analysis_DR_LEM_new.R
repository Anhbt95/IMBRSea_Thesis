...
spp <- obs %>% filter(NameEnglish == "Lemon sole")
...
# Check extreme values in L_wt_mean and Quota_uti
...
saveRDS(I1,file=paste0(dir_data,"/Data_model/","DR_LEM1_new.rds"))
...
saveRDS(I2,file=paste0(dir_data,"/Data_model/","DR_LEM2_new.rds"))
...
# I3 -------------------------------------
# I2 - CHL, SST, Price, Gravel
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(chl_year:price_eurobykg), -MudPercent
                         )), 
                         collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
                           f(w, model=spde) + f(Year, model = 'iid')")) #f(L_wt_mean_grp, model = 'rw2') + 
f
f3 <- "I + LU + logC + Q + D + S + G + B + rw(L) + V + T + W + Y"
...
saveRDS(I3,file=paste0(dir_data,"/Data_model/","DR_LEM3_new.rds"))
...
# I4 -------------------------------------
# I2 - CHL, SST, Price, Gravel, Quota_uti, Gravel, Bathy
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(chl_year:price_eurobykg), 
                                                -(MudPercent:Bathy), - Quota_uti
                         )), 
                         collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
...
saveRDS(I4,file=paste0(dir_data,"/Data_model/","DR_LEM4_new.rds"))
...
# I4.2 -------------------------------------
# I2 - CHL, SST, Price, Gravel, Quota_uti, Gravel, Bathy, Slope
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(chl_year:price_eurobykg), 
                                                -(MudPercent:Bathy), - Quota_uti, - Slope
                         )), 
                         collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
...
saveRDS(I4.2,file=paste0(dir_data,"/Data_model/","DR_LEM4.2_new.rds"))

ggregplot::Efxplot(list(I2))

round(I4.2$summary.fixed[,c("mean", "sd", "0.025quant", "0.975quant")],3)
...
saveRDS(I5,file=paste0(dir_data,"/Data_model/","DR_LEM5_new.rds"))

ggregplot::Efxplot(list(I2))

round(I5$summary.fixed[,c("mean", "sd", "0.025quant", "0.975quant")],4)
...
saveRDS(I6,file=paste0(dir_data,"/Data_model/","DR_LEM6_new.rds"))

ggregplot::Efxplot(list(I11))
round(I6$summary.fixed[,c("mean", "sd", "0.025quant", "0.975quant")],4)

I0 <- readRDS(file=paste0(dir_data,"/Data_model/","DR_LEM6_new.rds"))
...
lcpo <- c(-sum(log(I1$cpo$cpo)), -sum(log(I2$cpo$cpo)),
          ...
          -sum(log(I4.2$cpo$cpo)), -sum(log(I5$cpo$cpo)), 
          -sum(log(I6$cpo$cpo), na.rm = T)
)


Z     <- cbind(waic, lcpo)
rownames(Z) <- c(f1, f2, f3, f4, f4.2, f5, f6)
Z
...
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_LEM_Validation",".png"), scale=2,
       plot = last_plot(),
...
p1
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_LEM_Spatial_mean",".png"), scale=2,
       plot = last_plot(),
...
p2
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_LEM_Spatial_sd",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_LEM_Spatial",".png"), scale=2,
       plot = last_plot(),
...
p3
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_LEM_Pred_mean",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_LEM_Pred_sd",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p6
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_LEM_DR",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p7
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_LEM_CPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p5
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_LEM_DPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p8
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_LEM_LPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_LEM_Pred_all",".png"), scale=2,
       plot = last_plot(),
...
# save plots as .png
#ggsave(path = paste0(dir_figure,"/"),
#       filename = paste0("DR_LEM_Pred_all2",".png"), scale=2,
#       plot = last_plot(),
#       width = 22, height = 24, units = "cm",
#       dpi = 300
#)
...
# Predictors visualized: logCPUE, Slope, L_wt_mean

# DR = Intercept + logCPUE + SLope + L

b_I <-  I0$summary.fixed["Intercept",]
b_logC <- I0$summary.fixed["logCPUE",]
b_S <- I0$summary.fixed["Slope",]

mean_logC <- mean(spp$logCPUE)
mean_S <- mean(spp$Slope)

L_b <- I0$summary.random$L_wt_mean_grp
mean(spp$L_wt_mean)
mean(spp$L_wt_mean_grp) #26.44
L_b$ID #ID 39 L = 26.48
b_L <- L_b[39,] 
...
#linear predictor
g_mean <- b_I$mean + b_logC$mean*spp$logCPUE + b_S$mean*mean_S + b_L$mean
g_0.025 <- b_I$`0.025quant` + b_logC$`0.025quant`*spp$logCPUE + b_S$`0.025quant`*mean_S + b_L$`0.025quant`
g_0.975 <- b_I$`0.975quant` + b_logC$`0.975quant`*spp$logCPUE + b_S$`0.975quant`*mean_S + b_L$`0.975quant` 
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_LEM_Effect_logC",".png"), scale=2,
       plot = last_plot(),
...
for(i in 1:nrow(L_b)) {
  df_L[which(df_L$L_wt_mean_grp == L_b$ID[i]),"g_mean"] <-  b_I$mean  +  
    b_logC$mean*mean_logC + b_S$mean*mean_S + L_b[i,"mean"] 
  df_L[which(df_L$L_wt_mean_grp == L_b$ID[i]),"g_0.025"] <- b_I$`0.025quant`  + 
    b_logC$`0.025quant`*mean_logC + b_S$`0.025quant`*mean_S + L_b[i,"0.025quant"] 
  df_L[which(df_L$L_wt_mean_grp == L_b$ID[i]),"g_0.975"] <- b_I$`0.975quant`  + 
    b_logC$`0.975quant`*mean_logC + b_S$`0.975quant`*mean_S + L_b[i,"0.975quant"] 
} 
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_LEM_Effect_L",".png"), scale=2,
       plot = last_plot(),
       width = 7, height = 6, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_LEM_Effect_all",".png"), scale=2,
       plot = last_plot(),
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_LEM_Non-linear effect",".png"), scale=2,
       plot = last_plot(),
