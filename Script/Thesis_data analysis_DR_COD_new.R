...
spp <- obs %>% filter(NameEnglish == "Atlantic cod")
...
saveRDS(I1,file=paste0(dir_data,"/Data_model/","DR_COD1_new.rds"))
...
saveRDS(I2,file=paste0(dir_data,"/Data_model/","DR_COD2_new.rds"))
...
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, - MudPercent)), collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
                           f(w, model=spde) + f(Year, model = 'iid')")) #f(L_wt_mean_grp, model = 'rw2') + 
f
f3 <- "I + LU + logC + Q + D + CHL + SST + P + S + G + B + rw(L) + V + T + W + Y"
...
saveRDS(I3,file=paste0(dir_data,"/Data_model/","DR_COD3_new.rds"))

ggregplot::Efxplot(list(I2, I3))
...
# I4 -------------------------------------
# I2 Full - MudPercent, Quarter

str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, #- (chl_year:MudPercent),
                                                - (Quarter2:Quarter4) )), collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
                           f(w, model=spde) + f(Year, model = 'iid')")) #f(L_wt_mean_grp, model = 'rw2') + 
f

f4 <- "I + LU + logC + D + CHL + SST + P + S + G + B + rw(L) + V + T + W + Y"
...
saveRDS(I4,file=paste0(dir_data,"/Data_model/","DR_COD4_new.rds"))

ggregplot::Efxplot(list(I3, I4))
round(I4$summary.fixed[,c("mean", "sd", "0.025quant", "0.975quant")],3) 

# I5 -------------------------------------
# I2 Full - MudPercent, Quarter, chl, sst, Price, Slope, GravelPercent, Bathy, Quota_uti

str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,logCPUE:IcesDivision7H, 
                                                - (Quarter2:Quarter4) )), collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
                           f(w, model=spde) + f(Year, model = 'iid')")) #f(L_wt_mean_grp, model = 'rw2') + 
f
f5 <- "I + logC + D + rw(L) + V + T + W + Y"
...
saveRDS(I5,file=paste0(dir_data,"/Data_model/","DR_COD5_new.rds"))
round(I5$summary.fixed[,c("mean", "sd", "0.025quant", "0.975quant")],3) 
ggregplot::Efxplot(list(I4, I5))

# I6 -------------------------------------
# I2 Full - MudPercent, Quarter, chl, sst
...
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         ...
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           ...
                           f(w, model=spde) + f(Year, model = 'iid')")) #f(L_wt_mean_grp, model = 'rw2') + 
f
f6 <- "I + LU + logC + D + P + S + G + B + rw(L) + V + T + W + Y"
...
saveRDS(I6,file=paste0(dir_data,"/Data_model/","DR_COD6_new.rds"))

ggregplot::Efxplot(list(I4, I5, I6))

# I7 -------------------------------------
# I2 Full - MudPercent, chl, sst, Price, Slope, GravelPercent, Bathy, Quota_uti
...
saveRDS(I7,file=paste0(dir_data,"/Data_model/","DR_COD7_new.rds"))

ggregplot::Efxplot(list(I7))
summary(I7)
I7$waic$waic

# I8 -------------------------------------
# I2 Full - MudPercent, chl, sst, Price, Slope, GravelPercent, Quota_uti
# I7 + Bathy


str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,logCPUE:IcesDivision7H, Bathy
                         )), collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
                           f(w, model=spde) + f(Year, model = 'iid')")) #f(L_wt_mean_grp, model = 'rw2') + 
f
f8 <- "I + logC + Q + D + rw(L) + V + T + W + Y + B"
...
saveRDS(I8,file=paste0(dir_data,"/Data_model/","DR_COD8_new.rds"))

# I9 -------------------------------------
# I2 Full - MudPercent, chl, sst, Price, Slope, GravelPercent, Bathy
# I7 + Quota_uti

str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,logCPUE:IcesDivision7H, Quota_uti
                         )), collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
                           f(w, model=spde) + f(Year, model = 'iid')")) #f(L_wt_mean_grp, model = 'rw2') + 
f
f9 <- "I + logC + Q + D + B + rw(L) + V + T + W + Y + LU"
...
saveRDS(I9,file=paste0(dir_data,"/Data_model/","DR_COD9_new.rds"))
round(I9$summary.fixed[,c("mean", "sd", "0.025quant", "0.975quant")],3) 

# I10 -------------------------------------
# I2 Full - MudPercent, chl, sst, Price, Slope, GravelPercent, Bathy, Quarter
# I9 - Quarter
...
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         ...
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           ...
                           f(w, model=spde) + f(Year, model = 'iid')")) #f(L_wt_mean_grp, model = 'rw2') + 
f
f10 <- "I + logC + D + B + rw(L) + V + T + W + Y + LU"
...
saveRDS(I10,file=paste0(dir_data,"/Data_model/","DR_COD10_new.rds"))
round(I10$summary.fixed[,c("mean", "sd", "0.025quant", "0.975quant")],3) 

# I11 Prediction -----------------------------------------------------------
# The same as I9

# Prediction stack 
# Stack - no environmental variables are important so prediction is based on spatial effect only
...
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         ...
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           ...
                           f(w, model=spde) + f(Year, model = 'iid')")) #f(L_wt_mean_grp, model = 'rw2') + 
f
f11 <- "I + logC + Q + D + B + rw(L) + V + T + W + Y + LU"
...
saveRDS(I11,file=paste0(dir_data,"/Data_model/","DR_COD11_new.rds"))

ggregplot::Efxplot(list(I11))
summary(I8)

I0 <- readRDS(file=paste0(dir_data,"/Data_model/","DR_COD11_new.rds"))
...
lcpo <- c(-sum(log(I1$cpo$cpo)), -sum(log(I2$cpo$cpo)),
          ...
          -sum(log(I9$cpo$cpo)), -sum(log(I10$cpo$cpo)),
          -sum(log(I11$cpo$cpo), na.rm = T)
)


Z     <- cbind(waic, lcpo)
rownames(Z) <- c(f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, f11)
Z
summary(I9)
...
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_COD_Validation",".png"), scale=2,
       plot = last_plot(),
...
p1
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_COD_Spatial_mean",".png"), scale=2,
       plot = last_plot(),
...
p2
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_COD_Spatial_sd",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p3
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_COD_Pred_mean",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_COD_Pred_sd",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p5 <- ggplot() +
  geom_sf(data = aoi, fill = "grey", color = "white") +
  geom_point(data = spp %>% arrange(DPUE), 
             aes(x = HaulLongitude, 
                 y = HaulLatitude, 
                 size = DPUE, 
                 color = DPUE), 
...
p5
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_COD_DPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p6
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_COD_DR",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p7
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_COD_CPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p8
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_COD_LPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_COD_Pred_all",".png"), scale=2,
       plot = last_plot(),
...
#linear predictor
g_mean <- b_I$mean  + b_logC$mean*spp$logCPUE + b_L$mean
g_0.025 <- b_I$`0.025quant` + b_logC$`0.025quant`*spp$logCPUE + b_L$`0.025quant`
g_0.975 <- b_I$`0.975quant` + b_logC$`0.975quant`*spp$logCPUE + b_L$`0.975quant` 
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_COD_Effect_logC",".png"), scale=2,
       plot = last_plot(),
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_COD_Effect_L",".png"), scale=2,
       plot = last_plot(),
       width = 7, height = 6, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_COD_Effect_all",".png"), scale=2,
       plot = last_plot(),
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_COD_Non-linear effect",".png"), scale=2,
       plot = last_plot(),
