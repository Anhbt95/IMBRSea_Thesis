...
spp <- obs %>% filter(NameEnglish == "Whiting")
...
# Check extreme values in Quota_uti
ggplot(data = spp, aes(x = Quota_uti, y = DR)) + geom_point() + geom_smooth(method = "lm")
ggplot(data = spp %>% filter(Quota_uti < 2), aes(x = Quota_uti, y = DR)) + geom_point() + geom_smooth(method = "lm")
# points with Quota_uti > 2 should be eliminated

spp <- spp %>% filter(Quota_uti < 2)
...
saveRDS(I1,file=paste0(dir_data,"/Data_model/","DR_WHG1_new.rds"))
...
saveRDS(I2,file=paste0(dir_data,"/Data_model/","DR_WHG2_new.rds"))
...
# I3 -------------------------------------
# I2 - Quota_uti, CHL, SST, Price, Slope, Mud
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(chl_year:MudPercent), -Quota_uti
                         )), 
                         collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
                           f(w, model=spde) + f(Year, model = 'iid')")) #f(L_wt_mean_grp, model = 'rw2') + 
f
f3 <- "I + logC + Q + D + G + B + rw(L) + V + T + W + Y"
...
saveRDS(I3,file=paste0(dir_data,"/Data_model/","DR_WHG3_new.rds"))
...
# I4 -------------------------------------
# I2 - Quota_uti, CHL, SST, Price, Slope, Mud, Quarter, GravelPercent, Bathy
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(chl_year:Bathy), 
...
saveRDS(I4,file=paste0(dir_data,"/Data_model/","DR_WHG4_new.rds"))
...
# I5 -------------------------------------
# I2 - Quota_uti, CHL, SST, Price, Slope, Mud, Quarter, GravelPercent
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(chl_year:price_eurobykg), 
                                                -(MudPercent:GravelPercent), - Quota_uti, - Slope
                         )), 
                         collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
                           f(w, model=spde) + f(Year, model = 'iid')")) 
f
...
saveRDS(I5,file=paste0(dir_data,"/Data_model/","DR_WHG5_new.rds"))
...
# I6 -------------------------------------
# I2 - Quota_uti, CHL, SST, Price, Slope, Mud, Quarter, GravelPercent, Quarter
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(chl_year:GravelPercent), 
                                                - Quota_uti, -(Quarter2:Quarter4), Bathy
                         )), 
                         collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
...
saveRDS(I6,file=paste0(dir_data,"/Data_model/","DR_WHG6_new.rds"))
...
# Prediction stack 
# Stack - no env variable is important so prediction is based on spatial effect 
...
saveRDS(I7,file=paste0(dir_data,"/Data_model/","DR_WHG7_new.rds"))

ggregplot::Efxplot(list(I11))
round(I7$summary.fixed[,c("mean", "sd", "0.025quant", "0.975quant")],3)

I0 <- readRDS(file=paste0(dir_data,"/Data_model/","DR_WHG7_new.rds"))
...
lcpo <- c(-sum(log(I1$cpo$cpo)), -sum(log(I2$cpo$cpo)),
          ...
          -sum(log(I7$cpo$cpo), na.rm = T)
)


Z     <- cbind(waic, lcpo)
rownames(Z) <- c(f1, f2, f3, f4, f5, f6, f7)
Z

#I6 is the best and prediction model
#Set I0 as I6
I0 <- I7
...
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_WHG_Validation",".png"), scale=2,
       plot = last_plot(),
...
p1
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_WHG_Spatial_mean",".png"), scale=2,
       plot = last_plot(),
...
p2
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_WHG_Spatial_sd",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_WHG_Spatial",".png"), scale=2,
       plot = last_plot(),
...
p3
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_WHG_Pred_mean",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_WHG_Pred_sd",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p5
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_WHG_DPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p6
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_WHG_DR",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p7
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_WHG_CPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p8
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_WHG_LPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_WHG_Pred_all",".png"), scale=2,
       plot = last_plot(),
...
# save plots as .png
#ggsave(path = paste0(dir_figure,"/"),
#       filename = paste0("DR_WHG_Pred_all2",".png"), scale=2,
#       plot = last_plot(),
#       width = 22, height = 24, units = "cm",
#       dpi = 300
#)
...
# Predictors visualized: logCPUE, L_wt_mean

# DR = Intercept + logCPUE + Bathy + L
...
mean(spp$L_wt_mean)
mean(spp$L_wt_mean_grp) #26.73
L_b$ID #ID 26 L = 26.61
b_L <- L_b[26,] 
...
#linear predictor
g_mean <- b_I$mean + b_logC$mean*spp$logCPUE + b_B$mean*mean_B + b_L$mean
g_0.025 <- b_I$`0.025quant` + b_logC$`0.025quant`*spp$logCPUE + b_B$`0.025quant`*mean_B + b_L$`0.025quant`
g_0.975 <- b_I$`0.975quant` + b_logC$`0.975quant`*spp$logCPUE + b_B$`0.975quant`*mean_B + b_L$`0.975quant` 
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_WHG_Effect_logC",".png"), scale=2,
       plot = last_plot(),
...
for(i in 1:nrow(L_b)) {
  df_L[which(df_L$L_wt_mean_grp == L_b$ID[i]),"g_mean"] <-  b_I$mean + 
    b_logC$mean*mean_logC + b_B$mean*mean_B + L_b[i,"mean"] 
  df_L[which(df_L$L_wt_mean_grp == L_b$ID[i]),"g_0.025"] <- b_I$`0.025quant`  + 
    b_logC$`0.025quant`*mean_logC + b_B$`0.025quant`*mean_B + L_b[i,"0.025quant"] 
  df_L[which(df_L$L_wt_mean_grp == L_b$ID[i]),"g_0.975"] <- b_I$`0.975quant`  + 
    b_logC$`0.975quant`*mean_logC + b_B$`0.975quant`*mean_B + L_b[i,"0.975quant"] 
} 
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_WHG_Effect_L",".png"), scale=2,
       plot = last_plot(),
       width = 7, height = 6, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_WHG_Effect_all",".png"), scale=2,
       plot = last_plot(),
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_WHG_Non-linear effect",".png"), scale=2,
       plot = last_plot(),
