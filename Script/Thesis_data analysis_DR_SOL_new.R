...
spp <- obs %>% filter(NameEnglish == "Common sole")
...
# Small value in L_wt_mean
ggplot(data = spp, aes(x = L_wt_mean, y = DR)) + geom_point() + geom_smooth()
summary(spp$L_wt_mean)
#Eliminate 1 observation with 18.36 L_wt_mean

spp <- spp %>% filter(L_wt_mean > 18.36)
...
saveRDS(I1,file=paste0(dir_data,"/Data_model/","DR_SOL1_new.rds"))
...
saveRDS(I2,file=paste0(dir_data,"/Data_model/","DR_SOL2_new.rds"))
...
# I3 -------------------------------------
# I2 - CHL, SST, Mud, Gravel
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(chl_year:sst), -(MudPercent:GravelPercent)
                         )), 
                         collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
                           f(w, model=spde) + f(Year, model = 'iid')")) #f(L_wt_mean_grp, model = 'rw2') + 
f
f3 <- "I + LU + logC + Q + D + P + S + B + rw(L) + V + T + W + Y"
...
saveRDS(I3,file=paste0(dir_data,"/Data_model/","DR_SOL3_new.rds"))
...
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         ...
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           ...
                           f(w, model=spde) + f(Year, model = 'iid')")) #f(L_wt_mean_grp, model = 'rw2') + 
f
f4 <- "I + LU + logC + Q + D + P + B + rw(L) + V + T + W + Y"
...
saveRDS(I4,file=paste0(dir_data,"/Data_model/","DR_SOL4_new.rds"))
...
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         ...
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           ...
                           f(w, model=spde) + f(Year, model = 'iid')")) #f(L_wt_mean_grp, model = 'rw2') + 
f
f5 <- "I + LU + logC + Q + D + P + B + rw(L) + V + T + W + Y"
...
saveRDS(I5,file=paste0(dir_data,"/Data_model/","DR_SOL5_new.rds"))

ggregplot::Efxplot(list(I11))
summary(I8)

I0 <- readRDS(file=paste0(dir_data,"/Data_model/","DR_SOL5_new.rds"))
...
lcpo <- c(-sum(log(I1$cpo$cpo)), -sum(log(I2$cpo$cpo)),
          ...
          -sum(log(I5$cpo$cpo), na.rm = T)
)


Z     <- cbind(waic, lcpo)
rownames(Z) <- c(f1, f2, f3, f4, f5)
Z


#I11 is the best and prediction model
#Set I0 as I11
I0 <- I5
...
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SOL_Validation",".png"), scale=2,
       plot = last_plot(),
...
p1
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SOL_Spatial_mean",".png"), scale=2,
       plot = last_plot(),
...
p2
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SOL_Spatial_sd",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SOL_Spatial",".png"), scale=2,
       plot = last_plot(),
...
p3
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SOL_Pred_mean",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SOL_Pred_sd",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p5
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SOL_DPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p6
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SOL_DR",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p7
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SOL_CPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p8
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SOL_LPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SOL_Pred_all",".png"), scale=2,
       plot = last_plot(),
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SOL_Pred_all2",".png"), scale=2,
       plot = last_plot(),
...
# Predictors visualized: Quota_uti, logCPUE, price, Bathy, L_wt_mean
...
#linear predictor
g_mean <- b_I$mean  + b_Q$mean*spp$Quota_uti + b_logC$mean*mean_logC + b_P$mean*mean_P + b_B$mean*mean_B + b_L$mean
g_0.025 <- b_I$`0.025quant` + b_Q$`0.025quant`*spp$Quota_uti + b_logC$`0.025quant`*mean_logC + b_P$`0.025quant`*mean_P + b_B$`0.025quant`*mean_B + b_L$`0.025quant`
g_0.975 <- b_I$`0.975quant` + b_Q$`0.975quant`*spp$Quota_uti + b_logC$`0.975quant`*mean_logC + b_P$`0.975quant`*mean_P + b_B$`0.975quant`*mean_B + b_L$`0.975quant` 
...
p_Q
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SOL_Effect_LU",".png"), scale=2,
       plot = last_plot(),
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SOL_Effect_logC",".png"), scale=2,
       plot = last_plot(),
       width = 7, height = 6, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SOL_Effect_P",".png"), scale=2,
       plot = last_plot(),
       width = 7, height = 6, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SOL_Effect_B",".png"), scale=2,
       plot = last_plot(),
       width = 7, height = 6, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SOL_Effect_L",".png"), scale=2,
       plot = last_plot(),
       width = 7, height = 6, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SOL_Effect_all",".png"), scale=2,
       plot = last_plot(),
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SOL_Non-linear effect",".png"), scale=2,
       plot = last_plot(),
