...
spp <- obs %>% filter(NameEnglish == "European hake")
...
#1 small extreme point in L_wt_mean should be eliminated
ggplot(data = spp, aes(x = L_wt_mean, y = DR)) + geom_point() + geom_smooth()
spp <- spp %>% filter(L_wt_mean > 10)
...
saveRDS(I1,file=paste0(dir_data,"/Data_model/","DR_HKE1_new.rds"))
...
saveRDS(I2,file=paste0(dir_data,"/Data_model/","DR_HKE2_new.rds"))
...
# I3 -------------------------------------
# I2 - Quota_uti, CHL, SST, Price, Mud, Gravel
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(chl_year:GravelPercent), 
                                                -Quota_uti, Slope
                         )), 
                         collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           ...
                           f(w, model=spde) + f(Year, model = 'iid')")) #f(L_wt_mean_grp, model = 'rw2') + 
f
f3 <- "I + logC + Q + D + S + B + rw(L) + V + T + W + Y"
...
saveRDS(I3,file=paste0(dir_data,"/Data_model/","DR_HKE3_new.rds"))
...
# I4 -------------------------------------
# I2 - Quota_uti, CHL, SST, Price, Mud, Gravel, Quarter, Slope
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(chl_year:GravelPercent), 
                                                -Quota_uti, -(Quarter2:Quarter4) 
                         )), 
                         collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
...
saveRDS(I4,file=paste0(dir_data,"/Data_model/","DR_HKE4_new.rds"))
...
# I5 -------------------------------------
# I2 - Quota_uti, CHL, SST, Price, Mud, Gravel, Slope
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(chl_year:GravelPercent), 
                                                -Quota_uti,
...
saveRDS(I5,file=paste0(dir_data,"/Data_model/","DR_HKE5_new.rds"))
...
# I6 -------------------------------------
# I2 - Quota_uti, CHL, SST, Price, Mud, Gravel, Slope, Bathy
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(chl_year:GravelPercent), 
                                                -Quota_uti, - Bathy
...
saveRDS(I6,file=paste0(dir_data,"/Data_model/","DR_HKE6_new.rds"))
...
# I7 -------------------------------------
# I2 - Quota_uti, CHL, SST, Price, Mud, Gravel, Slope, Bathy, IcesDivision
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(chl_year:GravelPercent), 
                                                -Quota_uti, - Bathy, -(IcesDivision4C:IcesDivision8B)
                         )), 
                         collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
...
saveRDS(I7,file=paste0(dir_data,"/Data_model/","DR_HKE7_new.rds"))
...
# I8 -------------------------------------
# I2 - Quota_uti, CHL, SST, Price, Mud, Gravel, Slope, Bathy, IcesDivision, Quarter
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(chl_year:GravelPercent), 
                                                -Quota_uti, - Bathy, -(Quarter2:IcesDivision8B)
                         )), 
                         collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
...
saveRDS(I8,file=paste0(dir_data,"/Data_model/","DR_HKE8_new.rds"))

ggregplot::Efxplot(list(I2))

round(I8$summary.fixed[,c("mean", "sd", "0.025quant", "0.975quant")],4)

# I9 Prediction -----------------------------------------------------------
# The same as I6

# Prediction stack 
# Stack - no variable is important so prediction is based on spatial effect 
...
saveRDS(I9,file=paste0(dir_data,"/Data_model/","DR_HKE9_new.rds"))

ggregplot::Efxplot(list(I11))
round(I9$summary.fixed[,c("mean", "sd", "0.025quant", "0.975quant")],4)

I0 <- readRDS(file=paste0(dir_data,"/Data_model/","DR_HKE9_new.rds"))
...
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HKE_Validation",".png"), scale=2,
       plot = last_plot(),
...
p1
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HKE_Spatial_mean",".png"), scale=2,
       plot = last_plot(),
...
p2
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HKE_Spatial_sd",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HKE_Spatial",".png"), scale=2,
       plot = last_plot(),
...
p3
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HKE_Pred_mean",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HKE_Pred_sd",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p5
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HKE_DPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p6
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HKE_DR",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p7
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HKE_CPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p8
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HKE_LPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HKE_Pred_all",".png"), scale=2,
       plot = last_plot(),
...
# save plots as .png
#ggsave(path = paste0(dir_figure,"/"),
#       filename = paste0("DR_HKE_Pred_all2",".png"), scale=2,
#       plot = last_plot(),
#       width = 22, height = 24, units = "cm",
#       dpi = 300
#)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HKE_Effect_logC",".png"), scale=2,
       plot = last_plot(),
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HKE_Effect_L",".png"), scale=2,
       plot = last_plot(),
       width = 7, height = 6, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HKE_Effect_all",".png"), scale=2,
       plot = last_plot(),
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HKE_Non-linear effect",".png"), scale=2,
       plot = last_plot(),
