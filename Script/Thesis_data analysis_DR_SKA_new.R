...
spp <- obs %>% filter(NameEnglish == "Raja rays nei")
...
MyVar <- c("price_eurobykg", "Quota_uti", "logCPUE",
...
X0 <- model.matrix(~ -1 + Intercept + Quota_uti + logCPUE + 
                     ...
                     Year + Bathy_grp, data = spp)  #no L_wt_mean
...
f
f1 <- "I + LU + logC + Q + D + CHL + SST + P + S + M + G + B + V + T + W + Y"
...
saveRDS(I1,file=paste0(dir_data,"/Data_model/","DR_SKA1_new.rds"))
...
# I2 -------------------------------------
# I1 - CHL, SST, Price, Slope, Mud, Gravel
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
...
saveRDS(I2,file=paste0(dir_data,"/Data_model/","DR_SKA2_new.rds"))
...
saveRDS(I3,file=paste0(dir_data,"/Data_model/","DR_SKA3_new.rds"))
...
saveRDS(I4,file=paste0(dir_data,"/Data_model/","DR_SKA4_new.rds"))

ggregplot::Efxplot(list(I4))
round(I4$summary.fixed[,c("mean", "sd", "0.025quant", "0.975quant")],4)

I0 <- readRDS(file=paste0(dir_data,"/Data_model/","DR_SKA4_new.rds"))
...
lcpo <- c(-sum(log(I1$cpo$cpo)), -sum(log(I2$cpo$cpo)),
          -sum(log(I3$cpo$cpo)), -sum(log(I4$cpo$cpo), na.rm = T)
)

Z     <- cbind(waic, lcpo)
rownames(Z) <- c(f1, f2, f3, f4)
Z
...
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SKA_Validation",".png"), scale=2,
       plot = last_plot(),
...
# Intraclass effect
# Vessel
sigma_Vessel^2/(sigma_Vessel^2 + sigma_Vessel_TripID^2 + mean(VarY) + sigma_Year^2) 
# Vessel_trip
(sigma_Vessel^2 + sigma_Vessel_TripID^2)/(sigma_Vessel^2 + sigma_Vessel_TripID^2 + mean(VarY) + sigma_Year^2)
# Year
sigma_Year^2/(sigma_Vessel^2 + sigma_Vessel_TripID^2 + mean(VarY) + sigma_Year^2)
...
p1
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SKA_Spatial_mean",".png"), scale=2,
       plot = last_plot(),
...
p2
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SKA_Spatial_sd",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SKA_Spatial",".png"), scale=2,
       plot = last_plot(),
...
p3
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SKA_Pred_mean",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SKA_Pred_sd",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p5
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SKA_DPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p6
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SKA_DR",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p7
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SKA_CPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p8
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SKA_LPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SKA_Pred_all",".png"), scale=2,
       plot = last_plot(),
...
# save plots as .png
#ggsave(path = paste0(dir_figure,"/"),
#       filename = paste0("DR_SKA_Pred_all2",".png"), scale=2,
#       plot = last_plot(),
#       width = 22, height = 24, units = "cm",
#       dpi = 300
#)
...
# Predictors visualized: logCPUE

# DR = Intercept + logCPUE
...
#linear predictor
g_mean <- b_I$mean + b_logC$mean*spp$logCPUE
g_0.025 <- b_I$`0.025quant` + b_logC$`0.025quant`*spp$logCPUE
g_0.975 <- b_I$`0.975quant` + b_logC$`0.975quant`*spp$logCPUE
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_SKA_Effect_logC",".png"), scale=2,
       plot = last_plot(),
