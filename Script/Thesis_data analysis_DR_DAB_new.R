...
spp <- obs %>% filter(NameEnglish == "Common dab")
...
saveRDS(I1,file=paste0(dir_data,"/Data_model/","DR_DAB1_new.rds"))
...
saveRDS(I2,file=paste0(dir_data,"/Data_model/","DR_DAB2_new.rds"))
...
# I3 -------------------------------------
# I2 - IcesDivision, CHL, SST, Price, Slope, MudPercent
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(IcesDivision4C:MudPercent)
                         )), 
                         collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
                           f(w, model=spde) + f(Year, model = 'iid')")) #f(L_wt_mean_grp, model = 'rw2') + 
f
f3 <- "I + LU + logC + Q + G + B + rw(L) + V + T + W + Y"
...
saveRDS(I3,file=paste0(dir_data,"/Data_model/","DR_DAB3_new.rds"))
...
# I4 -------------------------------------
# I2 - IcesDivision, CHL, SST, Price, Slope, MudPercent, Quota_uti, Bathy
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(IcesDivision4C:MudPercent), 
                                                - Quota_uti, - Bathy
...
saveRDS(I4,file=paste0(dir_data,"/Data_model/","DR_DAB4_new.rds"))
...
# I5 -------------------------------------
# I2 - IcesDivision, CHL, SST, Price, Slope, MudPercent, Quota_uti
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(IcesDivision4C:MudPercent), 
                                                - Quota_uti
...
saveRDS(I5,file=paste0(dir_data,"/Data_model/","DR_DAB5_new.rds"))
...
# I5.2 -------------------------------------
# I2 - IcesDivision, CHL, SST, Price, Slope, MudPercent, Quota_uti, Year
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(IcesDivision4C:MudPercent), 
                                                - Quota_uti
                         ...
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
                           f(w, model=spde)")) #f(L_wt_mean_grp, model = 'rw2') + f(Year, model = 'iid')"
f
f5.2 <- "I + logC + G + B + rw(L) + V + T + W"
...
saveRDS(I5.2,file=paste0(dir_data,"/Data_model/","DR_DAB5.2_new.rds"))
...
saveRDS(I6,file=paste0(dir_data,"/Data_model/","DR_DAB6_new.rds"))

ggregplot::Efxplot(list(I11))
round(I6$summary.fixed[,c("mean", "sd", "0.025quant", "0.975quant")],3)

I0 <- readRDS(file=paste0(dir_data,"/Data_model/","DR_DAB6_new.rds"))
...
lcpo <- c(-sum(log(I1$cpo$cpo)), -sum(log(I2$cpo$cpo)),
          ...
          -sum(log(I5$cpo$cpo)), -sum(log(I5.2$cpo$cpo)), 
          -sum(log(I6$cpo$cpo), na.rm = T)
)


Z     <- cbind(waic, lcpo)
rownames(Z) <- c(f1, f2, f3, f4, f5, f5.2, f6)
Z
...
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_DAB_Validation",".png"), scale=2,
       plot = last_plot(),
...
p1
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_DAB_Spatial_mean",".png"), scale=2,
       plot = last_plot(),
...
p2
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_DAB_Spatial_sd",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_DAB_Spatial",".png"), scale=2,
       plot = last_plot(),
...
p3
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_DAB_Pred_mean",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_DAB_Pred_sd",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p5
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_DAB_DPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p6
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_DAB_DR",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p7
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_DAB_CPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p8
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_DAB_LPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_DAB_Pred_all",".png"), scale=2,
       plot = last_plot(),
...
# save plots as .png
#ggsave(path = paste0(dir_figure,"/"),
#       filename = paste0("DR_DAB_Pred_all2",".png"), scale=2,
#       plot = last_plot(),
#       width = 22, height = 24, units = "cm",
#       dpi = 300
#)
...
# Predictors visualized: logCPUE, Gravel, Bathy, L_wt_mean

# DR = Intercept + logCPUE + Gravel + Bathy + L

b_I <-  I0$summary.fixed["Intercept",]
b_logC <- I0$summary.fixed["logCPUE",]
b_G <- I0$summary.fixed["GravelPercent",]
b_B <- I0$summary.fixed["Bathy",]


mean_logC <- mean(spp$logCPUE)
mean_G <- mean(spp$GravelPercent)
mean_B <- mean(spp$Bathy)

L_b <- I0$summary.random$L_wt_mean_grp
mean(spp$L_wt_mean)
mean(spp$L_wt_mean_grp) #21.53
L_b$ID #ID 31 L = 21.537
b_L <- L_b[31,] 
...
#linear predictor
g_mean <- b_I$mean + b_logC$mean*spp$logCPUE + b_G$mean*mean_G + b_B$mean*mean_B + b_L$mean
g_0.025 <- b_I$`0.025quant` + b_logC$`0.025quant`*spp$logCPUE + b_G$`0.025quant`*mean_G + b_B$`0.025quant`*mean_B + b_L$`0.025quant`
g_0.975 <- b_I$`0.975quant` + b_logC$`0.975quant`*spp$logCPUE + b_G$`0.975quant`*mean_G + b_B$`0.975quant`*mean_B + b_L$`0.975quant` 
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_DAB_Effect_logC",".png"), scale=2,
       plot = last_plot(),
...
#linear predictor
g_mean <- b_I$mean + b_logC$mean*mean_logC + b_G$mean*spp$GravelPercent + b_B$mean*mean_B + b_L$mean
g_0.025 <- b_I$`0.025quant` + b_logC$`0.025quant`*mean_logC + b_G$`0.025quant`*spp$GravelPercent + b_B$`0.025quant`*mean_B + b_L$`0.025quant`
g_0.975 <- b_I$`0.975quant` + b_logC$`0.975quant`*mean_logC + b_G$`0.975quant`*spp$GravelPercent + b_B$`0.975quant`*mean_B + b_L$`0.975quant` 
...
df_G <- data.frame(
  ...
  mu_0.025 = mu_0.025,
  mu_0.975 = mu_0.975,
  GravelPercent = spp$GravelPercent
)

p_G <- ggplot() + geom_point(data = df_G, 
                             aes(x = GravelPercent, y = fitted), 
                             alpha = 0.2, color = "grey") +
  geom_line(data = df_G, 
            aes(x = GravelPercent, y = mu_mean), 
            colour = "black",
            lwd = 1) +
  geom_ribbon(data = df_G, 
              aes(x = GravelPercent, ymax = mu_0.975, ymin = mu_0.025), 
              alpha = 0.5) + 
  labs(x = "Gravel ratio",
       y = "Fitted ratio") +
  theme_bw() +
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_DAB_Effect_G",".png"), scale=2,
       plot = last_plot(),
       width = 7, height = 6, units = "cm",
       dpi = 300
)
...
#linear predictor
g_mean <- b_I$mean + b_logC$mean*mean_logC + b_G$mean*mean_G + b_B$mean*spp$Bathy + b_L$mean
g_0.025 <- b_I$`0.025quant` + b_logC$`0.025quant`*mean_logC + b_G$`0.025quant`*mean_G + b_B$`0.025quant`*spp$Bathy + b_L$`0.025quant`
g_0.975 <- b_I$`0.975quant` + b_logC$`0.975quant`*mean_logC + b_G$`0.975quant`*mean_G + b_B$`0.975quant`*spp$Bathy + b_L$`0.975quant` 
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_DAB_Effect_B",".png"), scale=2,
       plot = last_plot(),
       width = 7, height = 6, units = "cm",
       dpi = 300
)
...
for(i in 1:nrow(L_b)) {
  df_L[which(df_L$L_wt_mean_grp == L_b$ID[i]),"g_mean"] <-  b_I$mean  +  
    b_logC$mean*mean_logC + b_G$mean*mean_G + b_B$mean*mean_B + L_b[i,"mean"] 
  df_L[which(df_L$L_wt_mean_grp == L_b$ID[i]),"g_0.025"] <- b_I$`0.025quant`  + 
    b_logC$`0.025quant`*mean_logC + b_G$`0.025quant`*mean_G + b_B$`0.025quant`*mean_B + L_b[i,"0.025quant"] 
  df_L[which(df_L$L_wt_mean_grp == L_b$ID[i]),"g_0.975"] <- b_I$`0.975quant`  + 
    b_logC$`0.975quant`*mean_logC + b_G$`0.975quant`*mean_G + b_B$`0.975quant`*mean_B + L_b[i,"0.975quant"] 
} 
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_DAB_Effect_L",".png"), scale=2,
       plot = last_plot(),
       width = 7, height = 6, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_DAB_Effect_all",".png"), scale=2,
       plot = last_plot(),
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_DAB_Non-linear effect",".png"), scale=2,
       plot = last_plot(),
