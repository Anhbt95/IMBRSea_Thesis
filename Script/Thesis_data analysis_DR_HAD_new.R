...
spp <- obs %>% filter(NameEnglish == "Haddock")
...
MyVar <- c("L_wt_mean", "price_eurobykg", "Quota_uti", "logCPUE",
           "Bathy", "Slope", "GravelPercent", "SandPercent", "chl_year", "sst")
corvif(spp[,MyVar]) 
...
MyVar <- c("L_wt_mean", "price_eurobykg", "Quota_uti", "logCPUE",
           "Bathy", "Slope", "GravelPercent", "MudPercent", "chl_year", "sst")
corvif(spp[,MyVar])
...
# Potential predictors need smooth function: L_wt_mean
...
#   DR_tran_ijkst ~ beta(Pi_ijkst, phi) (Pij is mu)

#   E[DR_tran_ijkst] = Pi_ijkst

#                           Pi_ijkst * (1 - Pi_ijkst)
#   var[DR_tran_ijkst] = ..............................
#                                phi + 1

#   logit(Pi_ijkst) = I + LU + logC + Q + D + CHL + SST + P + S + M + G + B + rw(L) +
#                   V_j + T_jk + W_s + Y_t

# I = Intercept, LU = Landing limitation utilization, logC = log10 CPUE, 
# P = Price, Q = Quarter, D = ICES Division, S = Slope, M = Mud percentage, 
# G = Gravel percentage, CHL = Chlorophyll-a concentration, SST = Sea surface temperature, 
# B = Bathymetry, L = Weighted mean length, 
# V = Vessel random effect, T = Trip random effect, W = Spatial effect, Y = Temporal effect (Year),
# rw() = non-linear random walk 2 model


# DR_tran_ijkst is the observed discard ratio at Haul i, Vessel j, Trip ij, location s, year t

#   Priors (Default)  

#   Fixed parameters (Beta)
#   Bo ~ N(0,inf) (precision tau = 0)
#   B_n ~ N(0,31.6^2) (precision tau = 0.001)

#   Random paramters
#   L ~ N(0, sigma_L^2)
#   V_j ~ N(0, sigma_Vessel^2) 
#   T_jk ~ N(0, sigma_Trip^2) 
#   W_s ~ GMRF(0, covariance matrix SIGMA) (GMRF - Gaussian Markov Random Field)
#   Y_t ~ N(0, sigma_Year^2)
...
#   log(tau_Trip) ~ LogGamma(1, 0.00005)
#   log(tau_Year) ~ LogGamma(1, 0.00005)
#   log(tau_L) ~ LogGamma(1, 0.00005) #https://inla.r-inla-download.org/r-inla.org/doc/latent/rw2.pdf
...
sort(unique(spp$L_wt_mean_grp)) #1 cm
...
saveRDS(I1,file=paste0(dir_data,"/Data_model/","DR_HAD1_new.rds"))
...
saveRDS(I2,file=paste0(dir_data,"/Data_model/","DR_HAD2_new.rds"))

ggregplot::Efxplot(list(I2))

round(I2$summary.fixed[,c("mean", "sd", "0.025quant", "0.975quant")],5) 

I2$waic$waic
I1$waic$waic

# I3 -------------------------------------
# I2 - CHL, SST, Slope, MudPercent, GravelPercent
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(chl_year:sst), -(Slope:GravelPercent)
                         )), 
                         collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
                           f(w, model=spde) + f(Year, model = 'iid')")) #f(L_wt_mean_grp, model = 'rw2') + 
f
f3 <- "I + LU + logC + Q + D + P + B + rw(L) + V + T + W + Y"
...
saveRDS(I3,file=paste0(dir_data,"/Data_model/","DR_HAD3_new.rds"))
...
# I4 -------------------------------------
# I2 - CHL, SST, Slope, MudPercent, GravelPercent, Quarter, IcesDivision
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(chl_year:sst), 
                                                -(Slope:GravelPercent), -(Quarter2:IcesDivision8B)
                         )), 
                         collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
...
saveRDS(I4,file=paste0(dir_data,"/Data_model/","DR_HAD4_new.rds"))
...
# I5 -------------------------------------
# I2 - CHL, SST, Slope, MudPercent, GravelPercent, Quarter, IcesDivision, Quota_uti
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(chl_year:sst), 
                                                -(Slope:GravelPercent), -(Quarter2:IcesDivision8B), - Quota_uti
                         )), 
                         collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
...
saveRDS(I5,file=paste0(dir_data,"/Data_model/","DR_HAD5_new.rds"))
...
# I6 -------------------------------------
# I2 - CHL, SST, Slope, MudPercent, GravelPercent, IcesDivision, Quota_uti
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(chl_year:sst), 
                                                -(Slope:GravelPercent), -(IcesDivision4C:IcesDivision8B), - Quota_uti
                         )), 
                         collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
...
saveRDS(I6,file=paste0(dir_data,"/Data_model/","DR_HAD6_new.rds"))

ggregplot::Efxplot(list(I2))

round(I6$summary.fixed[,c("mean", "sd", "0.025quant", "0.975quant")],4)

# I7 -------------------------------------
# I2 - CHL, SST, Slope, MudPercent, GravelPercent, Quarter
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(chl_year:sst), 
                                                -(Slope:GravelPercent), -(Quarter2:Quarter4)
                         )), 
                         collapse = " + "),
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           f(L_wt_mean_grp, model = 'rw2') +
                           f(w, model=spde) + f(Year, model = 'iid')")) #f(L_wt_mean_grp, model = 'rw2') + 
f
f7 <- "I + LU + logC + D + P + B + rw(L) + V + T + W + Y"
...
saveRDS(I7,file=paste0(dir_data,"/Data_model/","DR_HAD7_new.rds"))

ggregplot::Efxplot(list(I2))

round(I7$summary.fixed[,c("mean", "sd", "0.025quant", "0.975quant")],4)

# I8 -------------------------------------
# I2 - CHL, SST, Slope, MudPercent, GravelPercent, Quarter, Year
str(X.est)
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         paste0(colnames(select(X.est,Quota_uti:Bathy, -(chl_year:sst), 
                                                -(Slope:GravelPercent), -(Quarter2:Quarter4)
...
saveRDS(I8,file=paste0(dir_data,"/Data_model/","DR_HAD8_new.rds"))
...
# Prediction stack 
# Stack - Bathy variable is important so prediction is based on spatial effect and Bathy
...
f   <- as.formula(paste0("y ~ -1 + Intercept + ", 
                         ...
                         " + f(Vessel, model = 'iid') + f(Vessel_TripID, model = 'iid') + 
                           ...
                           f(w, model=spde) + f(Year, model = 'iid')")) #f(L_wt_mean_grp, model = 'rw2') + 
f
f9 <- "I + LU + logC + D + P + B + rw(L) + V + T + W + Y"
...
saveRDS(I9,file=paste0(dir_data,"/Data_model/","DR_HAD9_new.rds"))

ggregplot::Efxplot(list(I11))
round(I9$summary.fixed[,c("mean", "sd", "0.025quant", "0.975quant")],4)

I0 <- readRDS(file=paste0(dir_data,"/Data_model/","DR_HAD9_new.rds"))
...
lcpo <- c(-sum(log(I1$cpo$cpo)), -sum(log(I2$cpo$cpo)),
          ...
          -sum(log(I7$cpo$cpo)), -sum(log(I8$cpo$cpo)),
          -sum(log(I9$cpo$cpo), na.rm = T)
)

Z     <- cbind(waic, lcpo)
rownames(Z) <- c(f1, f2, f3, f4, f5, f6, f7, f8, f9)
Z

#I9 is the best and prediction model
#Set I0 as I6
I0 <- I9
...
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HAD_Validation",".png"), scale=2,
       plot = last_plot(),
...
p1
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HAD_Spatial_mean",".png"), scale=2,
       plot = last_plot(),
...
p2
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HAD_Spatial_sd",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HAD_Spatial",".png"), scale=2,
       plot = last_plot(),
...
#legend_break <- round(seq(-0.05, 0.4, length.out = 8), 2)
...
p3
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HAD_Pred_mean",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HAD_Pred_sd",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
summary(spp$DPUE)
hist(spp$DPUE, breaks = 100)
mybreaks <- c(1,10,100,200)

p5 <- ggplot() +
  geom_sf(data = aoi, fill = "grey", color = "white") +
  geom_point(data = spp %>% arrange(desc(DPUE)), 
             aes(x = HaulLongitude, 
                 y = HaulLatitude, 
                 size = DPUE, 
                 color = DPUE), 
...
p5
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HAD_DPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p6
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HAD_DR",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p7 <- ggplot() +
  geom_sf(data = aoi, fill = "grey", color = "white") +
  geom_point(data = spp %>% arrange(desc(CPUE)), 
             aes(x = HaulLongitude, 
                 y = HaulLatitude, 
                 size = CPUE, 
                 color = CPUE), 
...
p7
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HAD_CPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
p8 <- p8 + 
  ...
  scale_color_viridis(name="LPUE", trans="log1p", breaks=mybreaks) + #log1p - log(1+x) as LPUE has 0
  guides(colour = guide_legend()) + 
  coord_sf(xlim = xlim, ylim = ylim, expand = FALSE) +
  labs(title = "LPUE",
       x = NULL,
...
p8
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HAD_LPUE",".png"), scale=2,
       plot = last_plot(),
       width = 11, height = 12, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HAD_Pred_all",".png"), scale=2,
       plot = last_plot(),
...
# save plots as .png
#ggsave(path = paste0(dir_figure,"/"),
#       filename = paste0("DR_HAD_Pred_all2",".png"), scale=2,
#       plot = last_plot(),
#       width = 22, height = 24, units = "cm",
#       dpi = 300
#)
...
#linear predictor
g_mean <- b_I$mean + b_Q$mean*mean_Q + b_logC$mean*spp$logCPUE + b_P$mean*mean_P + b_B$mean*mean_B + b_L$mean
g_0.025 <- b_I$`0.025quant` + b_Q$`0.025quant`*mean_Q + b_logC$`0.025quant`*spp$logCPUE + b_P$`0.025quant`*mean_P + b_B$`0.025quant`*mean_B + b_L$`0.025quant`
g_0.975 <- b_I$`0.975quant` + b_Q$`0.975quant`*mean_Q + b_logC$`0.975quant`*spp$logCPUE + b_P$`0.975quant`*mean_P + b_B$`0.975quant`*mean_B + b_L$`0.975quant`
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HAD_Effect_logC",".png"), scale=2,
       plot = last_plot(),
...
#linear predictor
g_mean <- b_I$mean + b_Q$mean*mean_Q + b_logC$mean*mean_logC + b_P$mean*spp$price_eurobykg + b_B$mean*mean_B + b_L$mean
g_0.025 <- b_I$`0.025quant` + b_Q$`0.025quant`*mean_Q + b_logC$`0.025quant`*mean_logC + b_P$`0.025quant`*spp$price_eurobykg + b_B$`0.025quant`*mean_B + b_L$`0.025quant`
g_0.975 <- b_I$`0.975quant` + b_Q$`0.975quant`*mean_Q + b_logC$`0.975quant`*mean_logC + b_P$`0.975quant`*spp$price_eurobykg + b_B$`0.975quant`*mean_B + b_L$`0.975quant`
...
df_P <- data.frame(
  ...
  mu_0.025 = mu_0.025,
  mu_0.975 = mu_0.975,
  price_eurobykg = spp$price_eurobykg
)

p_P <- ggplot() + geom_point(data = df_P, 
                             aes(x = price_eurobykg, y = fitted), 
                             alpha = 0.2, color = "grey") +
  geom_line(data = df_P, 
            aes(x = price_eurobykg, y = mu_mean), 
            colour = "black",
            lwd = 1) +
  geom_ribbon(data = df_P, 
              aes(x = price_eurobykg, ymax = mu_0.975, ymin = mu_0.025), 
              alpha = 0.5) + 
  labs(x = "Fish price (EUR/kg)",
       y = "Fitted ratio") +
  theme_bw() +
  theme(axis.text = element_text(size = rel(1),
                                 family = "Calibri"),
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HAD_Effect_P",".png"), scale=2,
       plot = last_plot(),
       width = 7, height = 6, units = "cm",
       dpi = 300
)
...
#linear predictor
g_mean <- b_I$mean + b_Q$mean*mean_Q + b_logC$mean*mean_logC + b_P$mean*mean_P + b_B$mean*spp$Bathy + b_L$mean
g_0.025 <- b_I$`0.025quant` + b_Q$`0.025quant`*mean_Q + b_logC$`0.025quant`*mean_logC + b_P$`0.025quant`*mean_P + b_B$`0.025quant`*spp$Bathy + b_L$`0.025quant`
g_0.975 <- b_I$`0.975quant` + b_Q$`0.975quant`*mean_Q + b_logC$`0.975quant`*mean_logC + b_P$`0.975quant`*mean_P + b_B$`0.975quant`*spp$Bathy + b_L$`0.975quant`
...
p_B <- ggplot() + geom_point(data = df_B, 
                             aes(x = Bathy, y = fitted), 
                             alpha = 0.2, color = "grey") +
  geom_line(data = df_B, 
            aes(x = Bathy, y = mu_mean), 
            colour = "black",
            lwd = 1) +
  geom_ribbon(data = df_B, 
              aes(x = Bathy, ymax = mu_0.975, ymin = mu_0.025), 
              alpha = 0.5) + 
  labs(x = "Bathymetry (m)",
       y = "Fitted ratio") +
  theme_bw() +
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HAD_Effect_B",".png"), scale=2,
       plot = last_plot(),
       width = 7, height = 6, units = "cm",
       dpi = 300
)
...
for(i in 1:nrow(L_b)) {
  df_L[which(df_L$L_wt_mean_grp == L_b$ID[i]),"g_mean"] <-  b_I$mean + 
    b_Q$mean*mean_Q + b_logC$mean*mean_logC + b_P$mean*mean_P + b_B$mean*mean_B + L_b[i,"mean"] 
  df_L[which(df_L$L_wt_mean_grp == L_b$ID[i]),"g_0.025"] <- b_I$`0.025quant`  + 
    b_Q$`0.025quant`*mean_Q + b_logC$`0.025quant`*mean_logC + b_P$`0.025quant`*mean_P + b_B$`0.025quant`*mean_B + L_b[i,"0.025quant"] 
  df_L[which(df_L$L_wt_mean_grp == L_b$ID[i]),"g_0.975"] <- b_I$`0.975quant`  + 
    b_Q$`0.975quant`*mean_Q + b_logC$`0.975quant`*mean_logC + b_P$`0.975quant`*mean_P + b_B$`0.975quant`*mean_B + L_b[i,"0.975quant"] 
} 
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HAD_Effect_L",".png"), scale=2,
       plot = last_plot(),
       width = 7, height = 6, units = "cm",
       dpi = 300
)
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HAD_Effect_all",".png"), scale=2,
       plot = last_plot(),
...
# save plots as .png
ggsave(path = paste0(dir_figure,"/"),
       filename = paste0("DR_HAD_Non-linear effect",".png"), scale=2,
       plot = last_plot(),
